...
small future engine-option or build change doesn't flap the suite. Calls
target native contract v2's ``render(buffer, sample_rate, markers,
pitch_scale, preserve_formants, quality)`` with the plain two-marker case
(``[[0, 0], [frames, target_frames]]``, ``pitch_scale=1.0``,
``preserve_formants=False``, ``quality="high"``) this step implements.
...
def test_supported_quality_names_engine_capability() -> None:
    ...
    # "high"; see test_unimplemented_option_raises_value_error below).
...
def test_unimplemented_option_raises_value_error(
    ...
    with pytest.raises(ValueError, match="not implemented yet"):
        rb.render(buf, SR, markers, pitch_scale, preserve_formants, quality)
